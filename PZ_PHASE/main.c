...
uint32_t U_summ = 0; // сума квадратів вхідної напруги
uint32_t U_summTriac = 0; // сума квадратів вихідної напруги
uint8_t triacON = FALSE;// чи ввімкнений тріак
uint16_t cnt_adc = 0; // лічильник вимірювань
uint16_t U_real = 0; // реальна напруга
uint16_t U_Out = 0, U_Out_Sum = 0, Temp_U_Out = 0; // напруга на виході
uint8_t u_set = 0;// напуга,яку будемо встановлювати.
...
uint16_t angleU = MAX_ANGLE; //кут відкриття симістора
...
uint8_t r_whole, r_dec; // ціла і дробова частина опору ТЕНа
...
__EEPROM_DATA(24, 20, 90, 0, 0, 0, 0, 0); // ініціалізація еепром, 
// 0 - ціла частина опору тена
// 1 - дробова частина опору тена
// 2 - напруга після навантаження
// 3 -
...
void main(void) {
   ...
   // EncData = U_Out;
    ...
    U_Out = read_eep(EE_ADR_LOAD_POWER); // початкове навантаження з еепром
    EncData = U_Out;
...
//        if(enTmp)
            ...
            if (TempEn) { // якщо  дозволено аварійне відключення при перевищенні температури
                if ((temperature / 10) >= AlarmTemp) { // якщо температура перевищила ліміт
                    TMR1ON = 0;
                    TRIAC = 0;
                    ; // вимикаємо навантаження до перезапуску, або натискання кнопки
                power_100 = 0;
        ...
        //опитування енкодера
        count_enc += EncPoll();
        if (((count_enc % 4) == 0) && !(count_enc == 0)) { // це всьо для енкодера
        ...
        if (!getU && (zero == ZERO)) {
            ...
            U_real = (uint16_t)sqrt(sqrtU_sum); // корінь числа, напруга на вході

            sqrtU_sum = U_summTriac / cnt_adc; // ділимо суму квадратів на кількість вимірювань
            U_Out = (uint16_t)sqrt(sqrtU_sum); // корінь числа, напруга на виході

            U_summTriac = 0;
            U_summ = 0; // 
            getU = TRUE; // дозволяємо записувати дані з АЦП


            if (u_set > U_real) {// якщо вихідна напруга менеше встановленої
                is_stab = 0; // не можливо стабілізувати
            } else
                is_stab = 1;
            
            if(powerOFF)
                TMR1ON = 0; //вимкнути навантаження
            ...
            if (power_100)
                watt_disp = calc_power(t_res, U_real); // розрахунок даних потужності, для відображення
            else
                watt_disp = calc_power(t_res, U_Out); // розрахунок даних потужності, для відображення
        }
        ...
        switch (sel_main) {
            case SEL_MAIN:
                ...
                if (pressed_key == KEY_OK_EVENT) { // якщо натиснули кнопку енкодера
                    //initLCD();
                    lcd_preload();
                    U_Out = (uint16_t)EncData; // ставимо в рег. потужності, то що накрутили
                    is_power = 1;
                    power_100 = 0;
                    powerOFF = 0;
                    //TMR1ON = 1;
                ...
                if (pressed_key == KEY_OK_EVENT) { // якщо натиснули кнопку енкодера
                    switch (EncData) {
                        ...
                        case 1: // запишемо в еепром дані поточного навантаження
                            ...
                            write_eep(EE_ADR_LOAD_POWER, U_Out); // поточна потужність в еепром
                            EncData = U_Out;
                            clearLCD();
                            break;
                        case 2: // 100% потужності
                            sel_main = SEL_MAIN; // будемо переходити в основне меню
                            clearLCD();
                            power_100 = 1; // ввімкнемо ТЕН на повну
                            EncData = U_Out;
                            show_tp = 0;
                            break;
...
void init_cpu(void) {
    ...
    T0CONbits.TMR0ON = 1;
    INTCON2bits.TMR0IP = 0; // низький пріоритет від таймера 0(1 - високий)
    //TMR0L = TMR0L_V; //  
    ...
    TMR3IP = 0; // низький пріоритет
    TMR3IE = 0; // переривання від таймера 3 вимкнені
    TMR3ON = 1;
    
    //compare
    CCP1CONbits.CCP1M = 0b1010; // 1010 = Compare mode, generate software interrupt only, CCP1 pin reverts to I/O state
    PIE1bits.CCP1IE = 1; 
    IPR1bits.CCP1IP = 1; // високий пріоритет
    SETCCP1(MAX_ANGLE);
    
    
        //compare CCP2 модуль буде запускати АЦП
...
//    PIE2bits.CCP2IE = 1; 
//    IPR2bits.CCP2IP = 1; // високий пріоритет
    SETCCP2(100);  // 50мкс
    
    
    // переривання від таймера 2 - для опитування ds18b20
...
void show_lcd_main(void) {
    ...
    if (!is_stab) {
        lcd_gotoxy(12, 1); // будемо виводити, що стабілізація
...
//
    if ((tick_t1_1 <= 100) && (show_tp)) { // показуємо, тимчасово, потужність
        //tick_t1_1 = 0;
